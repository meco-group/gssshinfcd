...
% along with gssshinfcd. If not, see <https://www.gnu.org/licenses/>.
    ...
    methods
        ...
        % WATCHDOG Constructor
            obj.printer = gssshinfcd.watchdog.printer(); 
        end
        ...
        % ERROR Throws a formatted error
            ...
            err = struct(); 
            err.message = 'gssshinfcd detected an error in the execution of your command. See the former error message or the logbook for more information.'; 
            err.identifier = 'gssshinfcd:watchdogError'; 
            err.stack.file = '';
            err.stack.name = 'gssshinfcd';
            err.stack.line = 1;
            error(err); 
        end
