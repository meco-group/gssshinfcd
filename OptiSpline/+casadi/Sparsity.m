classdef  Sparsity < casadi.SharedObject & casadi.SparsityInterfaceCommon & casadi.PrintableCommon
  ...
  methods
    ...
    %
    %
      [varargout{1:nargout}] = casadiMEX(75, self, varargin{:});
    end
    ...
    %
    %
      out = casadiMEX(94, self, varargin{:});
      if nargout<=1
        varargout{1}=out;
      else
    ...
    %LDL Symbolic LDL factorization Returns the sparsity pattern of L as well as the
    %
    %  [Sparsity , [int] OUTPUT] = LDL(self)
    %
    %elimination tree.
    %
    %
    ...
    %
    %  [Sparsity OUTPUT, Sparsity OUTPUT, [int] OUTPUT, [int] OUTPUT, [int] OUTPUT] = QR_SPARSE(self)
    %
    ...
    function varargout = symbfact(self,varargin)
    %SYMBFACT Symbolic factorization analysis See Direct Methods for Sparse Linear Systems
    %
    %  [[int] OUTPUT, [int] OUTPUT, [int] OUTPUT, Sparsity OUTPUT] = SYMBFACT(self, bool ata)
    %
    %by Davis (2006).
    %
    %
    ...
    %PRINT_COMPACT [DEPRECATED] Alias for disp
    %
    %  std::ostream & = PRINT_COMPACT(self)
    %
    %
    %
    %
  ...
  methods(Static)
    ...
    %
    %
     [varargout{1:nargout}] = casadiMEX(160, varargin{:});
    end
    ...
    %
    %
     [varargout{1:nargout}] = casadiMEX(162, varargin{:});
    end
